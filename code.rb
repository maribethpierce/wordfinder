...
def word_finder(str)
  letters = str.split('')
  key = "abcdefghijklmnopqrstuvwxyz_".split('')
  key.each do |character|
    count = 0
    letters.each do |x|
      if character == x
        count += 1
    ...
    @results.store(count, character)
    @solution = @results.sort.reverse.flatten
  end
  last_index = @solution.index("_")
  until @solution.length == last_index - 1
    @solution.pop
  end
  @word = Hash[*@solution].values.join("")
end

letter_collection = ("epqiiqwdiwgyka_vsqtsujeqqicnhyivo_sigwasmkwgsih_akl_gtnkhgikgveidpmt
